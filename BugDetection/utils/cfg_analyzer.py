...
def get_pos_arg_var(pos,code_slice):
    ...
    arg_list = find_node_by_type(tree, "argument_list")
    ic(slice,arg_list)
    # idents = find_node_by_type(arg_list[pos], "identifier")
    idents = find_node_by_type(arg_list[0], "identifier")
    ic(idents)

    return get_node_content(
            idents[pos], slice) # if is struct, only return the struct name, not field.
    '''
    # Find the function call node
    function_call_node = find_node_by_type(tree, "call_expression")[0]
...
def check_variable_is_local(code, var):
    return var in get_local_vars(code)

def check_variable_is_para(code, var):
    return var in get_func_paras(code)
